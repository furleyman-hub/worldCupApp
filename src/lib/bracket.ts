 ...
 *
 * For the ACTUAL bracket the live feed fills in real names (resolved1/2),
 * which always win over computation. Computation covers the gap between a
 * group finishing and the feed maintainer updating placeholders, and powers
 * the user's PREDICTED bracket (via pickOutcome/pickWinner).
 */
export interface BracketResolution {
  ...
  /** the 8 third-place qualifiers in ranking order (for UI) */
...
export function resolveBracket(
  ...
  opts: {
    ...
    pickWinner?: (num: number) => string | undefined;
  } = {}
): BracketResolution {
  const { pickOutcome, pickWinner } = opts;
  const predicted = !!pickOutcome;
  const tables = groupTables(matches, pickOutcome);
  ...
  for (const g of Object.keys(tables)) {
    if (!groupComplete(matches, g, pickOutcome)) continue;
    const t = tables[g];
  ...
  let thirdQualifiers: { team: string; group: string }[] = [];
  if (thirds.length === 12) {
    thirds.sort(
      (a, b) =>
        b.row.pts - a.row.pts ||
  ...
  const winnerOf = (num: number): string | null => {
    if (predicted) {
  ...
  const resolveSlot = (m: MergedMatch, slot: string, side: 0 | 1, num: number): string | null => {
    ...
    if (wl) return wl[1] === 'W' ? winnerOf(Number(wl[2])) : loserOf(Number(wl[2]));
    return null;
  };
